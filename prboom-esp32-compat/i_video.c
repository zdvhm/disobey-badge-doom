...
#ifdef DOOM_STREAMING
	// Process keys from HTTP and Serial input
	doom_stream_process_keys();
#endif
}
...
#ifdef DOOM_STREAMING
	// Send frame to stream (8-bit indexed version for better compression)
	// Note: screens[0].data is 16-bit RGB565, we need the 8-bit indexed version
	// For now, stream the RGB565 data directly
	doom_stream_update_frame((const uint8_t *)scr, SCREENWIDTH, SCREENHEIGHT);
#endif
...
void I_SetPalette (int pal)
{
	...
	const byte * pal_start = palette + pal*(3*256);
	palette = pal_start;
	for (i=0; i<255 ; i++) {
...
#ifdef DOOM_STREAMING
	// Update streaming palette
	doom_stream_set_palette(pal_start);
#endif

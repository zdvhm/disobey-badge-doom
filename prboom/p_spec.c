...
// ESP32: Default animation list for vanilla DOOM when ANIMATED lump not available
typedef struct {
...
static const default_animdef_t default_animdefs[] = {
  // Flats
  {0, "NUKAGE3",  "NUKAGE1",  8},
  {0, "FWATER4",  "FWATER1",  8},
  {0, "SWATER4",  "SWATER1",  8},
...
{
  int         i;
  int         lump = W_CheckNumForName("ANIMATED"); // ESP32: Use CheckNumForName

  //  Init animation
  lastanim = anims;

  if (lump >= 0) {
    // ANIMATED lump found - use it (original prboom behavior)
    ...
    // ESP32: No ANIMATED lump - use built-in default list
    lprintf(LO_INFO, "P_InitPicAnims: Using built-in animation list\n");

    for (i=0 ; default_animdefs[i].istexture != -1 ; i++)
    {
      if (lastanim >= anims + maxanims)
      {
        size_t newmax = maxanims ? maxanims*2 : MAXANIMS;
      ...
      if (default_animdefs[i].istexture)
      {
        if (R_CheckTextureNumForName(default_animdefs[i].startname) == -1)
            continue;

        int endtex = R_CheckTextureNumForName(default_animdefs[i].endname);
        int starttex = R_CheckTextureNumForName(default_animdefs[i].startname);
        if (endtex == -1 || starttex == -1)
            continue;

        lastanim->picnum = endtex;
        lastanim->basepic = starttex;
      }
      else
      {
        if ((W_CheckNumForName)(default_animdefs[i].startname, ns_flats) == -1)
            continue;

        int endflat = (W_CheckNumForName)(default_animdefs[i].endname, ns_flats);
        int startflat = (W_CheckNumForName)(default_animdefs[i].startname, ns_flats);
        if (endflat == -1 || startflat == -1)
            continue;

        lastanim->picnum = endflat - firstflat;
        lastanim->basepic = startflat - firstflat;
      }

      lastanim->istexture = default_animdefs[i].istexture;
      lastanim->numpics = lastanim->picnum - lastanim->basepic + 1;

      if (lastanim->numpics < 2)
          continue; // Skip invalid animations instead of erroring

      lastanim->speed = default_animdefs[i].speed;
      lastanim++;
    }
  }
}

...
// ESP32: Default switch list for vanilla DOOM when SWITCHES lump not available
typedef struct {
  char name1[9];
  char name2[9];
  short episode;
} default_switchlist_t;

static const default_switchlist_t default_switches[] = {
  // Shareware/Registered Episode 1
  {"SW1BRCOM", "SW2BRCOM", 1},
  {"SW1BRN1",  "SW2BRN1",  1},
  {"SW1BRN2",  "SW2BRN2",  1},
  {"SW1BRNGN", "SW2BRNGN", 1},
...
void P_InitSwitchList(void)
{
  ...
  int lump = W_CheckNumForName("SWITCHES"); // ESP32: Use CheckNumForName instead

  if (lump >= 0) {
    // SWITCHES lump found - use it (original prboom behavior)
    ...
    // ESP32: No SWITCHES lump - use built-in default list
    lprintf(LO_INFO, "P_InitSwitchList: Using built-in switch list\n");
    
    for (i=0; default_switches[i].episode != 0; i++)
    {
      if (index+1 >= max_numswitches)
        switchlist = realloc(switchlist, sizeof *switchlist *
            (max_numswitches = max_numswitches ? max_numswitches*2 : 8));
      if (default_switches[i].episode <= episode)
      {
        int texture1, texture2;

        texture1 = R_CheckTextureNumForName(default_switches[i].name1);
        texture2 = R_CheckTextureNumForName(default_switches[i].name2);
        if (texture1 != -1 && texture2 != -1) {
          switchlist[index++] = texture1;
          switchlist[index++] = texture2;
        }

...
{
  int i;
  // ESP32: C_START/C_END markers are from prboom.wad which may not be loaded
  // Fall back to just using COLORMAP if markers don't exist
  firstcolormaplump = W_CheckNumForName("C_START");
  if (firstcolormaplump < 0) {
    // No prboom.wad - use standard COLORMAP only
    numcolormaps = 1;
    colormaps = Z_Malloc(sizeof(*colormaps) * numcolormaps, PU_STATIC, 0);
    colormaps[0] = (const lighttable_t *)W_CacheLumpName("COLORMAP");
    firstcolormaplump = W_GetNumForName("COLORMAP");
...
// ESP32: Modified to handle missing patches gracefully
void R_SetPatchNum(patchnum_t *patchnum, const char *name)
{
  int lumpnum = W_CheckNumForName(name);
  
  if (lumpnum < 0) {
    // Patch not found - set to zero/invalid values
    patchnum->width = 0;
    patchnum->height = 0;
